      ...
      <CardContent className="space-y-4 bg-sidebar-primary">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input id="productId" type="number" value={selectedProductId} onChange={e => setSelectedProductId(Number(e.target.value))} placeholder="Ingresa ID del producto" />
          </div>
          <div>
            <Label htmlFor="quantity">Cantidad Solicitada</Label>
            <Input id="quantity" type="number" value={requestedQuantity} onChange={e => setRequestedQuantity(Number(e.target.value))} placeholder="Cantidad a extraer" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4 flex-wrap">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={optimizationSettings.prioritizeExpiration} onChange={e => setOptimizationSettings({
              ...optimizationSettings,
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={optimizationSettings.allowNearExpiry} onChange={e => setOptimizationSettings({
              ...optimizationSettings,
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={optimizationSettings.enableCrossStockOptimization} onChange={e => setOptimizationSettings({
              ...optimizationSettings,
        ...
        <Button onClick={handleGeneratePlan} className="w-full" disabled={!selectedProductId || requestedQuantity <= 0}>
          Generar Plan FIFO
        </Button>

        {/* Mostrar recomendaciones */}
        {fifoRecommendation && <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-medium">Plan de Movimiento Generado</h3>
              <div className="flex items-center gap-2">
                {fifoRecommendation.canFulfill ? <Badge variant="default" className="bg-green-500">
            ...
            {/* Advertencias */}
            {fifoRecommendation.warnings.length > 0 && <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <h4 className="font-medium text-yellow-800 mb-2">Advertencias:</h4>
                <ul className="text-sm text-yellow-700 space-y-1">
                  {fifoRecommendation.warnings.map((warning: string, index: number) => <li key={index}>• {warning}</li>)}
                </ul>
              </div>}
            ...
            <div className="space-y-2">
              ...
              {fifoRecommendation.batches.map((batch: any, index: number) => <div key={batch.batchId} className="flex items-center justify-between p-3 border rounded-lg">
            ...
            {/* Botón de ejecución */}
            {fifoRecommendation.canFulfill && <Dialog open={showExecuteDialog} onOpenChange={setShowExecuteDialog}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Textarea id="reason" value={reason} onChange={e => setReason(e.target.value)} placeholder="Venta, transferencia, ajuste..." required />
                    </div>
                    <div>
                      <Label htmlFor="reference">Documento de Referencia</Label>
                      <Input id="reference" value={referenceDocument} onChange={e => setReferenceDocument(e.target.value)} placeholder="Factura, orden, etc." />
                    </div>
                    <div className="flex gap-2 justify-end">
                      <Button variant="outline" onClick={() => setShowExecuteDialog(false)}>
                        Cancelar
                      </Button>
                      <Button onClick={handleExecuteMovement} disabled={isExecuting || !reason}>
